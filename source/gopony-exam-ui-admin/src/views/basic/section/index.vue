    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['basic:section:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['basic:section:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="显示排序" prop="weigh">
          <el-input v-model="form.weigh" placeholder="请输入显示排序" />
        </el-form-item>
...
<script>
import { listSection, getSection, delSection, addSection, updateSection,changeSectionStatus } from "@/api/basic/section";
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要' + text + '"' + row.name + '"吗？').then(function() {
        return changeSectionStatus(row.id, row.status);
      }).then(() => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除单元编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('basic/section/export', {
        ...this.queryParams
